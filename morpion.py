...
def tour(grille, joueur, joueur1, joueur2):

    if joueur==1:

        joue = input(joueur1 + " joue (ligne puis colonne) : ")
        temp = joue.split()
    else:
        joue = input(joueur2 + " joue (ligne puis colonne) : ")
        temp = joue.split()
...
joueur = 1
joueur1=str(input("Username 1 Croix : "))
joueur2=str(input("Username 2 Rond : "))
grille=[" "," "," "," "," "," "," "," "," "]
afficher_grille(grille)
gagne=0
while gagne==0:
    tour(grille,joueur,joueur1, joueur2)
